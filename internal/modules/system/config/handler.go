...
func list(runtime *bootstrap.Runtime) httpx.HandlerFunc {
	return func(c *httpx.Context) {
		...
		query := runtime.DB.Model(&SystemConfig{}).Order("id DESC")
		if keyword != "" {
...
func deleteConfigs(runtime *bootstrap.Runtime) httpx.HandlerFunc {
	return func(c *httpx.Context) {
		...
		if err := runtime.DB.Where("id IN ?", ids).Delete(&SystemConfig{}).Error; err != nil {
			c.Error(err)
			return
		}
...
func ensureConfigKeyUnique(runtime *bootstrap.Runtime, currentID int64, key string) error {
	var count int64
	query := runtime.DB.Model(&SystemConfig{}).Where("config_key = ?", key)
	if currentID > 0 {
