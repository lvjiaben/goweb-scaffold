...
func TestRunnerRemoveAndRegenerateWithLock(t *testing.T) {
	...
	if !containsPath(removeResult.RemovedFiles, "internal/modules/app/demo_article/model.go") {
		t.Fatalf("expected model.go removed, got %v", removeResult.RemovedFiles)
	}

	modelPath := filepath.Join(runner.Runtime.RepoRoot, "internal/modules/app/demo_article/model.go")
	if _, err := os.Stat(modelPath); !os.IsNotExist(err) {
		t.Fatalf("expected model.go removed, stat err=%v", err)
	}
	lockPath := filepath.Join(runner.Runtime.RepoRoot, "internal/modules/app/demo_article/codegen.lock.json")
	...
	if !containsPath(regenerateResult.GeneratedFiles, "internal/modules/app/demo_article/model.go") {
...
func TestRunnerBatchGenerateContinuesOnError(t *testing.T) {
  ...
  "modules": [
	...
	noticePath := filepath.Join(runner.Runtime.RepoRoot, "internal/modules/app/demo_notice/module.go")
