...
func list(runtime *bootstrap.Runtime) httpx.HandlerFunc {
	return func(c *httpx.Context) {
		...
		query := runtime.DB.Model(&AdminMenu{}).Order("sort ASC, id ASC")
		if keyword != "" {
...
func options(runtime *bootstrap.Runtime) httpx.HandlerFunc {
	return func(c *httpx.Context) {
		var menus []AdminMenu
		if err := runtime.DB.Where("menu_type = ?", MenuTypeMenu).Order("sort ASC, id ASC").Find(&menus).Error; err != nil {
			c.Error(err)
			return
		}
...
func deleteMenus(runtime *bootstrap.Runtime) httpx.HandlerFunc {
	return func(c *httpx.Context) {
		...
		err := runtime.DB.Transaction(func(tx *gorm.DB) error {
			if err := tx.Where("menu_id IN ?", deleteIDs).Delete(&AdminRoleMenu{}).Error; err != nil {
				return err
			}
			return tx.Where("id IN ?", deleteIDs).Delete(&AdminMenu{}).Error
		})
		if err != nil {
			c.Error(err)
			return
